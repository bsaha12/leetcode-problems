        ...
        for (int i = row - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < col; j++) {
                for (int k = 0; k < col; k++) {
                    ...
                    if (col1 + 1 < col) {
                        next = Math.max(next, prev[col1 + 1][col2]);
                        if (col2 + 1 < col)
                            next = Math.max(next, prev[col1 + 1][col2 + 1]);
                        if (col2 - 1 >= 0)
                            next = Math.max(next, prev[col1 + 1][col2 - 1]);
                    }
                    ...
                    if (col2 + 1 < col) {
                        next = Math.max(next, prev[col1][col2 + 1]);
                        if (col1 - 1 >= 0)
                            next = Math.max(next, prev[col1 - 1][col2 + 1]);
                    }
                    ...
                    next = Math.max(next, prev[col1][col2]);

                    curr[j][k] = next + currVal;
                }
            }
            prev = curr;
        }
